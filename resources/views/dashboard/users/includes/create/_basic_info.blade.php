...
{{-- <div class="form-group my-4 row">
    ...
    <div class="col-sm-10">
        <input type="text" name="full_name" value="{{ old('full_name') }}" id="full_name" class="form-control @error('full_name') is-invalid @enderror" placeholder="@lang('main.full_name')" required>
        @error('full_name')
...
<div class="form-group mb-4 row">
    ...
    <div class="col-sm-10">
        <input type="email" name="email" value="{{ old('email') }}" id="email" class="form-control @error('email') is-invalid @enderror" placeholder="@lang('main.email')" required>
        @error('email')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
...
<div class="form-group mb-4 row">
    ...
    <div class="col-sm-10">
        <input type="password" name="password" value="{{ old('password') }}" id="password" class="form-control @error('password') is-invalid @enderror" placeholder="@lang('main.password')" required>
        @error('password')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
...
<div class="form-group mb-4 row">
    ...
    <div class="col-sm-10">
        <input type="text" name="phone" value="{{ old('phone') }}" id="phone" class="form-control @error('phone') is-invalid @enderror" placeholder="@lang('main.phone')" required>
        @error('phone')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
    ...
    <div class="col-sm-10">
        <select name="type" id="type" class="form-control select2 @error('type') is-invalid @enderror" data-placeholder="@lang('main.type')" required>
            ...
            @foreach ($userTypes as $type)
                <option value="{{ $type }}" {{ old('type') == $type ? 'selected' : '' }}> @lang("main.{$type}") </option>
            @endforeach
        </select>
        @error('type')
            <div class="invalid-feedback">
    ...
    <div class="col-sm-10">
        <select name="status" id="status" class="form-control select2 @error('status') is-invalid @enderror" data-placeholder="@lang('main.status')" required>
            ...
            @foreach ($userStatus as $status)
                <option value="{{ $status }}" {{ old('status') == $status ? 'selected' : '' }}> @lang("main.{$status}") </option>
            @endforeach
        </select>
        @error('status')
            <div class="invalid-feedback">
